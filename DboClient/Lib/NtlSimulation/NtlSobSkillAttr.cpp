...
void CNtlSobSkillAttr::HandleEvents(RWS::CMsg &pMsg)
{
	if(pMsg.Id == g_EventSobCreate)
	{
		...
		if(!pSkillTbl)
		{
			NtlLogFilePrint("CNtlSobSkillAttr::HandleEvents(RWS::CMsg &pMsg) Skill table is null");
			return;
		}
		NTL_ASSERT( pSkillTbl, "Skill table is null 1 (" << pSkillCreate->uiSkillTblId << ")" );
	...
	else if(pMsg.Id == g_EventSobSkillUpgrade)
	{
		...
		NTL_ASSERT( pSkillTbl, "Skill table is null 2 (" << pSkillUpgrade->uiUpgradeSkillTblId << ")" );
		SetSkillTbl( pSkillTbl );
