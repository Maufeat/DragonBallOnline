...
DEFINITION_MEMORY_POOL(CNtlBehaviorCharFly)

CNtlBehaviorCharFly::CNtlBehaviorCharFly()
{
...
void CNtlBehaviorCharFly::Enter(void)
{
    ChangeFlyState(JUMPSTATE_START);

    // 나중에 base class enter를 호출한다.
    CNtlBehaviorBase::Enter(); 
}

void CNtlBehaviorCharFly::Exit(void)
{
...
void CNtlBehaviorCharFly::Update(RwReal fElapsed)
{
...
RwUInt32 CNtlBehaviorCharFly::HandleEvents(RWS::CMsg &pMsg)
{
...
void CNtlBehaviorCharFly::UpdatePositionMove(SMoveStuff *pMoveStuff, SJumpStuff *pJumpStuff, RwV3d& vPos, RwBool bIncHeight, RwReal fElapsed, RwReal fSpeedScale /*= 1.0f*/)
{
    RwReal fOrgY = vPos.y;
...
RwBool CNtlBehaviorCharFly::UpdateMoveSync(RwV3d vPos, RwReal fElapsedTime)
{
    if(m_pActor->GetClassID() != SLCLASS_PLAYER)
        return FALSE;
...
void CNtlBehaviorCharFly::UpdateJumpStart(SMoveStuff *pMoveStuff, SJumpStuff *pJumpStuff, RwReal fElapsed)
{
...
void CNtlBehaviorCharFly::UpdateJumpLoop(SMoveStuff *pMoveStuff, SJumpStuff *pJumpStuff, RwReal fElapsed)
{
    RwV3d vPos = m_pActor->GetPosition();
    ...
    if(pMoveStuff->byMoveFlags != NTL_MOVE_NONE)
    {
        if(vNextPos.y <= m_sHStuff.fFinialHeight + 0.5f || m_byCollMoveImpossCnt >= COLLISION_MOVE_IMPOSSIBLE_COUNT)
        {
            if(Logic_IsSwimming(m_pActor, &vPos, m_sHStuff.fWorldHeight, m_sHStuff.fWaterHeight))
                ChangeFlyState(JUMPSTATE_WATER_LANDING);
            else
                ChangeFlyState(JUMPSTATE_MOVE_LANDING);
        }
    }
    else
    {
        if(vNextPos.y <= m_sHStuff.fFinialHeight + 0.5f || m_byCollMoveImpossCnt >= COLLISION_MOVE_IMPOSSIBLE_COUNT)
        {
            if(Logic_IsSwimming(m_pActor, &vPos, m_sHStuff.fWorldHeight, m_sHStuff.fWaterHeight))
                ...
                ChangeFlyState(JUMPSTATE_STAND_LANDING);
        }
    }
...
void CNtlBehaviorCharFly::UpdateJumpStandLanding(SMoveStuff *pMoveStuff, SJumpStuff *pJumpStuff, RwReal fElapsed)
{
...
void CNtlBehaviorCharFly::UpdateJumpMoveLanding(SMoveStuff *pMoveStuff, SJumpStuff *pJumpStuff, RwReal fElapsed)
{
    RwV3d vPos = m_pActor->GetPosition();
...
void CNtlBehaviorCharFly::UpdateJumpWaterLanding(SMoveStuff *pMoveStuff, SJumpStuff *pJumpStuff, RwReal fElapsed)
{
    RwV3d vPos = m_pActor->GetPosition();
...
void CNtlBehaviorCharFly::ChangeFlyState(RwUInt8 byJumpState)
{
    CNtlBeCharData *pBeData = reinterpret_cast<CNtlBeCharData*>(m_pActor->GetBehaviorData());     
    SJumpStuff *pJumpStuff = pBeData->GetJumpStuff();
...
void CNtlBehaviorCharFly::UpdateSendSyncCheck( RwReal fElapsedTime )
{
