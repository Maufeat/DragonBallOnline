...
void CNtlSobItemAttr::HandleEvents(RWS::CMsg &pMsg)
{
	if(pMsg.Id == g_EventSobCreate)
	{
		...
		if( !pSobItemCreate->bNeedToIdentify )
		{
			if(!pItemTbl)
			{
				NtlLogFilePrint("CNtlSobItemAttr::HandleEvents(RWS::CMsg &pMsg) pItemTbl table is null");
				return;
			}
			m_pItemTbl = reinterpret_cast<sITEM_TBLDAT*>( pItemTbl->FindData(pSobItemCreate->uiItemTblId) );
			if(!m_pItemTbl)
			{
				NtlLogFilePrint("CNtlSobItemAttr::HandleEvents(RWS::CMsg &pMsg) m_pItemTbl table is null");
				return;
			}
